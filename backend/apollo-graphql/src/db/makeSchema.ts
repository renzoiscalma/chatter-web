import mongoose, { Schema, model, connection, Model, Types, Document} from "mongoose";
...
async function run () {
	...
	if (userOne?._id && userTwo?._id)
		await MessageCollection.insertMany(generateInitialMessage(lobby._id, userOne._id, userTwo._id));
