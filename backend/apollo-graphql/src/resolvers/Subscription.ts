		...
		messageAdded: { // todo types
			subscribe: withFilter(
				...
				(payload: any, variables: any) => {
					console.log("hey", payload.messageAdded, "zzz", payload.messageAdded.messages[0].from.id);
					console.log(payload.lobbyId, variables, typeof(payload.lobbyId), typeof(variables.lobbyId));
					return payload.messageAdded.lobbyId === variables.lobbyId && variables.userId !== payload.messageAdded.messages[0].from.id;
				}
			)
